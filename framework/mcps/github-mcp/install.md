...
Copy [`configs/opencode.json`](./configs/opencode.json) into your OpenCode project root or user config directory, ensure `GITHUB_TOKEN` is set in your `.env` file, and restart the OpenCode agent.
...
- **Authentication errors**: confirm the PAT has the required scopes and `GITHUB_TOKEN` is set in your `.env` or environment.
